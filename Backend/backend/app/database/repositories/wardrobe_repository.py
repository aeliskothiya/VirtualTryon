...
def get_wardrobe_item_for_user(db: Database, item_id: str, user_id: str) -> dict | None:
    return db.wardrobe_items.find_one({"_id": to_object_id(item_id), "user_id": user_id, "status": {"$ne": "inactive"}})


def list_wardrobe_items_for_user(db: Database, user_id: str) -> list[dict]:
    return list(db.wardrobe_items.find({"user_id": user_id, "status": {"$ne": "inactive"}}).sort("created_at", -1))
