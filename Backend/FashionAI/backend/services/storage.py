...
def create_profile_photo_path(user_id: int, filename: str | None) -> Path:
    return settings.user_media_root / str(user_id) / f"profile{_safe_extension(filename)}"


def create_override_photo_path(user_id: int, filename: str | None) -> Path:
    token = uuid4().hex[:12]
    return settings.user_media_root / str(user_id) / f"override_{token}{_safe_extension(filename)}"


def create_wardrobe_item_path(user_id: int, filename: str | None) -> Path:
    token = uuid4().hex[:12]
    return settings.wardrobe_media_root / str(user_id) / f"{token}{_safe_extension(filename)}"


def create_tryon_output_path(user_id: int, job_id: int) -> Path:
